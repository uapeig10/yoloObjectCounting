...
weightsPath = os.path.sep.join([args["yolo"], "yolov3-custom.weights"])
...
while True:
        ...
        for box in boxes:
            ...
            if indexIDs[i] in previous:
                ...
                if intersect(p0, p1, line1[0], line1[1]):
                    counter[classIDs[i]] += 1

            text = "{}: {:.4f}".format(LABELS[classIDs[i]], confidences[i])
            #text = "{}".format(indexIDs[i])
            cv2.putText(frame, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2)
            i += 1
    ...
    # draw counter
    counter_text = "counterLight:{}".format(counter[0])
    cv2.putText(frame, counter_text, (100,100), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 0, 255), 2)
    counter_text = "counterHeavy:{}".format(counter[1])
    cv2.putText(frame, counter_text, (100,140), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 0, 255), 2)
    counter_text = "counterBike:{}".format(counter[2])
    cv2.putText(frame, counter_text, (100,180), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 0, 255), 2)
